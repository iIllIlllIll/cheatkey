        ...
        try:
            ...
            if order is None:
                ...
                if long_holding is False:
                    if ((now.minute) % 15 == 4 or (now.minute)%15 == 5) and f.cheatkey(symbol, interval='5m', threshold=CHEATKEY_THRESHOLD, lookback_n=CHEATKEY_LOOKBACK, use_time_filter=CHEATKEY_TIMEFILTER, side='long'):
                        # 롱 초기매수
...
'''
                ...
                if short_holding is False:
                    if ((now.minute) % 15 == 4 or (now.minute)%15 == 5) and f.cheatkey(symbol, interval='5m', threshold=CHEATKEY_THRESHOLD, lookback_n=CHEATKEY_LOOKBACK, use_time_filter=CHEATKEY_TIMEFILTER, side='short'):
                        # 숏 초기매수
...
'''
            ...
            if order is None:
                if long_holding is True:
                    ...
                    elif long_count > 1 and long_pnl <= LONG_ADD_SL_PNL[long_count]:
                        order = f.close(symbol,side='long')
                        long_savemode = True
                    ...
                    # Partial Exit Add/All
                        ...
                        if PARTIAL_EXIT_MODE == 'added':
                            ...
                            order = f.close_usdt(symbol,usdt=long_qty,side='long')
                    ...
                    # Exit Add/All
                        ...
                        and f.cheatkey(
                            ...
                            use_time_filter=CHEATKEY_TIMEFILTER,
                            side='short'
                        ):
            ...
            if order is None:
                if short_holding is True:
                    ...
                    # SL Exit
                    elif short_pnl <= SHORT_SL_PNL:
                        order = f.close(symbol, side='short')
                        short_savemode = True

                    elif short_count > 1 and short_pnl <= SHORT_ADD_SL_PNL[short_count]:
                        order = f.close(symbol, side='short')
                        short_savemode = True
                    ...
                    # Partial Exit Add/All
                        ...
                        if PARTIAL_EXIT_MODE == 'added':
                            ...
                            order = f.close_usdt(symbol, usdt=short_qty, side='short')
            ...
            if now.minute == 10:  # 정시(00분)인지 확인
                ...
                if short_holding is False:
                    ...
                    short_status = '🟢매수중'
                blnc = f.get_futures_asset_balance(asset="USDT")
                long_count = len(long_buytime_list) if long_buytime_list else 0
                short_count = len(short_buytime_list) if short_buytime_list else 0
